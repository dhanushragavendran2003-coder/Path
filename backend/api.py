...
# Add production URL if provided in environment
prod_url = os.environ.get('PRODUCTION_FRONTEND_URL')
if prod_url:
...
def load_artifacts():
    ...
    # Use absolute paths relative to this file to ensure loading works everywhere
    current_dir = os.path.dirname(os.path.abspath(__file__))
    base_path = os.path.join(current_dir, 'models')
    
    try:
        MODEL = joblib.load(os.path.join(base_path, 'rf_model.pkl'))
        ...
        print(f"[SUCCESS] Class: {type(MODEL)}")
        print(f"[SUCCESS] Features expected: {FEATURE_NAMES}")
...
def predict():
    ...
    try:
        ...
        for feature in FEATURE_NAMES:
            if feature not in features_dict:
                ...
                try:
                    ...
                    print(f"[ERROR] Invalid value for feature {feature}: {features_dict[feature]}")
        ...
        print(f"[SUCCESS] Prediction successful: {predicted_role} ({max_confidence:.1f}%)")
